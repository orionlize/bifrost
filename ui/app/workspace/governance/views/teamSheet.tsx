...
export default function TeamSheet({ team, customers, onSave, onCancel }: TeamSheetProps) {
	...
	const validator = useMemo(() => {
		...
		return new Validator([
			Validator.required(formData.name.trim(), t("teams.nameRequired")),
			Validator.custom(formData.isDirty, t("teams.noChanges")),
			...budgetValidators,
			Validator.custom(uniqueDurations === populatedDurations.length, t("teams.distinctBudgetDurations")),

			// Rate limit validation - token limits
			...(formData.tokenMaxLimit !== undefined && formData.tokenMaxLimit !== null
				? [
						Validator.minValue(tokenMaxLimitNum || 0, 1, t("teams.tokenMin")),
						Validator.required(formData.tokenResetDuration, t("teams.tokenResetRequired")),
					]
				: []),
			...
			...(formData.requestMaxLimit !== undefined && formData.requestMaxLimit !== null
				? [
						Validator.minValue(requestMaxLimitNum || 0, 1, t("teams.requestMin")),
						Validator.required(formData.requestResetDuration, t("teams.requestResetRequired")),
					]
				: []),
		]);
		...
		<Sheet open onOpenChange={(open) => !open && onCancel()}>
			...
			>
				<SheetHeader className="flex flex-col items-start px-8 py-4" headerClassName="mb-0 sticky -top-4 bg-card z-10">
					<SheetTitle className="flex items-center gap-2">{isEditing ? t("teams.editTitle") : t("teams.createTitle")}</SheetTitle>
					<SheetDescription>{isEditing ? t("teams.editDescription") : t("teams.createDescription")}</SheetDescription>
				</SheetHeader>
					...
					<div className="grow space-y-6 px-8">
						...
						<div className="flex flex-col gap-6">
							<div className="space-y-2">
								<Label htmlFor="name">{t("teams.sheet.teamNameLabel")}</Label>
								<Input
									id="name"
									placeholder={t("teams.sheet.teamNamePlaceholder")}
									value={formData.name}
									maxLength={50}
							...
							{customers?.length > 0 && (
								<div className="space-y-2">
									<Label htmlFor="customer">{t("teams.sheet.customerOptional")}</Label>
									<Select
									...
									>
										<SelectTrigger id="customer" className="w-full" data-testid="team-customer-select-trigger">
											<SelectValue placeholder={t("teams.selectCustomer")} />
										</SelectTrigger>
										<SelectContent>
									...
									<p className="text-muted-foreground text-sm">{t("teams.sheet.assignCustomerHint")}</p>
								</div>
							)}
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label>{t("teams.sheet.budgets")}</Label>
								<button
									type="button"
									...
									<div className="flex items-start gap-2">
										<div className="flex-1">
											<NumberAndSelect
												...
												label={t("teams.sheet.budgetRowLabel", { index: idx + 1 })}
												value={row.maxLimit}
								...
								<div className="flex items-center justify-between gap-4 rounded-md border px-3 py-2">
									<div className="space-y-0.5">
										<Label htmlFor="team-calendar-aligned-toggle" className="text-sm font-normal">
											{t("teams.sheet.alignCalendar")}
										</Label>
										<p className="text-muted-foreground text-xs">{t("teams.sheet.alignCalendarDesc")}</p>
									</div>
									<Switch
						...
						<AlertDialog open={showCalendarAlignWarning} onOpenChange={setShowCalendarAlignWarning}>
							<AlertDialogContent>
								<AlertDialogHeader>
									<AlertDialogTitle>{t("teams.sheet.calendarResetTitle")}</AlertDialogTitle>
									<AlertDialogDescription>{t("teams.sheet.calendarResetDesc")}</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
									...
									>
										{t("teams.sheet.enableCalendarAlignment")}
									</AlertDialogAction>
								</AlertDialogFooter>
							</AlertDialogContent>
						</AlertDialog>
						...
						{isEditing && ((team?.budgets && team.budgets.length > 0) || team?.rate_limit) && (
							<div className="bg-muted/50 space-y-4 rounded-lg border p-4">
								<p className="text-sm font-medium">{t("teams.sheet.currentUsage")}</p>
								<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
									{team?.budgets?.map((b) => (
										<div key={b.id} className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("teams.sheet.budgetUsage", { duration: b.reset_duration })}</p>
									...
									{team?.rate_limit?.token_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("teams.sheet.tokens")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
											...
											<p className="text-muted-foreground text-xs">
												{t("teams.sheet.lastReset")} {formatDistanceToNow(new Date(team.rate_limit.token_last_reset), { addSuffix: true })}
											</p>
										</div>
									)}
									{team?.rate_limit?.request_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("teams.sheet.requests")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<span className="inline-block">
											<Button type="submit" disabled={loading || !validator.isValid() || !hasPermission} data-testid="team-save-btn">
												{loading ? t("common.actions.saving") : isEditing ? t("teams.updateTeam") : t("teams.addTeam")}
											</Button>
										</span>
									</TooltipTrigger>
									{(loading || !validator.isValid() || !hasPermission) && (
										<TooltipContent>
											<p>
												{!hasPermission
													? t("teams.noPermission")
													: loading
														? t("common.actions.saving")
														: validator.getFirstError() || t("teams.fixValidation")}
											</p>
										</TooltipContent>
									)}
								</Tooltip>
