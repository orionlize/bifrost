			...
			{ocrInput && (
				<div className="w-full rounded-sm border">
					<div className="flex items-center gap-2 border-b px-6 py-2 text-sm font-medium">
						...
						{t("logsMedia.ocrInput")}
					</div>
					<div className="space-y-4 p-6">
						<div>
							<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logsMedia.fieldType")}</div>
							<div className="font-mono text-xs">{ocrInput.type === "document_url" ? t("logsMedia.document") : t("logsMedia.image")}</div>
						</div>
						{(ocrInput.document_url || ocrInput.image_url) && (
							<div>
								<div className="text-muted-foreground mb-2 text-xs font-medium">
									{ocrInput.type === "document_url" ? t("logsMedia.documentUrl") : t("logsMedia.imageUrl")}
								</div>
				...
				<div className="w-full rounded-sm border">
					...
					<div className="space-y-4 p-6">
						{ocrOutput.usage_info && (
							<div className="grid grid-cols-3 gap-3">
								<div className="space-y-1">
									<div className="text-muted-foreground text-xs font-medium">{t("logsMedia.pagesProcessed")}</div>
									<div className="font-mono text-xs">{ocrOutput.usage_info.pages_processed}</div>
								</div>
								<div className="space-y-1">
									<div className="text-muted-foreground text-xs font-medium">{t("logsMedia.documentSize")}</div>
						...
						{ocrOutput.document_annotation && (
							<div>
								<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logsMedia.documentAnnotation")}</div>
						...
						{currentPage && (
							<>
								{currentPage.dimensions && (
									<div className="grid grid-cols-3 gap-3">
										<div className="space-y-1">
											<div className="text-muted-foreground text-xs font-medium">{t("logsMedia.dimensions")}</div>
											<div className="font-mono text-xs">
												{currentPage.dimensions.width} × {currentPage.dimensions.height}px
											</div>
										</div>
										<div className="space-y-1">
											<div className="text-muted-foreground text-xs font-medium">{t("logsMedia.dpi")}</div>
								...
								{currentPage.markdown ? (
									<div>
										<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logsMedia.markdown")}</div>
										<CodeEditor
									...
									<div className="text-muted-foreground font-mono text-xs">{t("logsMedia.noTextExtracted")}</div>
								)}

								{pageImages.length > 0 && (
									<div>
										<div className="text-muted-foreground mb-2 text-xs font-medium">
											{t("logsMedia.extractedImagesCount", { count: pageImages.length })}
										</div>
								...
								{totalPages > 1 && (
									<div className="mt-3 flex items-center justify-center gap-4">
										<Button
											...
											aria-label={t("logsMedia.previousPage")}
											title={t("logsMedia.previousPage")}
											data-testid="ocr-view-pagination-prev-button"
										>
											<ChevronLeft className="h-4 w-4" />
										</Button>
										<span className="text-muted-foreground text-sm">
											{t("logsMedia.pageIndicator", { current: currentIndex + 1, total: totalPages })}
										</span>
										<Button
											variant="outline"
											size="sm"
											onClick={goToNext}
											aria-label={t("logsMedia.nextPage")}
											title={t("logsMedia.nextPage")}
