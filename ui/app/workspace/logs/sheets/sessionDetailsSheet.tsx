...
export function SessionDetailsSheet({
	...
	const summaryCards: SummaryCard[] = useMemo(
		() => [
			{
				label: t("logsSession.logs"),
				value: (sessionSummary?.count || 0).toLocaleString(),
				helper:
					sessionSummary && sessionLogs.length < sessionSummary.count
						? t("logsSession.loaded", { count: sessionLogs.length.toLocaleString() })
						: undefined,
			},
			{
				label: t("logsSession.totalCost"),
				value: `$${(sessionSummary?.total_cost || 0).toFixed(4)}`,
			},
			{
				label: t("logsSession.totalTokens"),
				value: (sessionSummary?.total_tokens || 0).toLocaleString(),
			},
			{
				label: t("logsSession.started"),
				value: sessionSummary?.started_at ? format(new Date(sessionSummary.started_at), "MMM d, yyyy hh:mm:ss aa") : t("logsMedia.na"),
				size: "sm",
			},
			{
				label: t("logsSession.latestUpdate"),
				value: sessionSummary?.latest_at ? format(new Date(sessionSummary.latest_at), "MMM d, yyyy hh:mm:ss aa") : t("logsMedia.na"),
				size: "sm",
			},
			{
				label: t("logsSession.duration"),
	...
	const loadSessionPage = useCallback(
		async (offset: number, reset = false) => {
			...
			try {
				...
				if (result.error) {
					toast.error(t("logsSession.loadFailed"), {
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-full flex-col gap-4 overflow-x-hidden p-8 sm:max-w-[60%]">
				<div className="flex items-center justify-between gap-4">
					<div>
						<div className="text-lg font-medium">{t("logsSession.title")}</div>
						{sessionId && onFilterByParentRequestId ? (
							<Tooltip>
								...
								<TooltipContent sideOffset={6}>{t("logsSession.filterSession")}</TooltipContent>
							</Tooltip>
						) : (
					...
					<div className="flex items-center gap-3">
						...
						>
							...
							{sortOrder === "asc" ? t("logsSession.sortEarliest") : t("logsSession.sortLatest")}
						</Button>
					</div>
				</div>
					...
					<Table containerClassName="h-full overflow-auto">
						<TableHeader className="sticky top-0 z-10 bg-[#f9f9f9] dark:bg-[#27272a]">
							<TableRow>
								...
								<TableHead>{t("logsSession.time")}</TableHead>
								<TableHead>{t("logsSession.type")}</TableHead>
								<TableHead>{t("logsSession.message")}</TableHead>
								<TableHead>{t("logsSession.provider")}</TableHead>
								<TableHead>{t("logsSession.model")}</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{loadingSession && sessionLogs.length === 0 ? (
								...
								sessionLogs.map((log) => (
									...
									>
										...
										<TableCell className="max-w-[140px] truncate font-mono text-xs">{log.model || t("logsMedia.na")}</TableCell>
									</TableRow>
								))
							) : (
								<TableRow>
									<TableCell colSpan={6} className="text-muted-foreground h-24 text-center">
										{t("logsSession.noLogs")}
									</TableCell>
								</TableRow>
							)}
						</TableBody>
