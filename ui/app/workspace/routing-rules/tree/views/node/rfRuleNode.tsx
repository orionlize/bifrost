...
import { RULE_W, SCOPE_CONFIG, SCOPE_LABEL_KEYS, type ScopeKey } from "../constants";
			...
			>
				...
				<div className={`flex items-center gap-1.5 rounded-t-[6px] px-3 py-1.5 ${cfg?.headerClass ?? "bg-gray-100 dark:bg-gray-800/30"}`}>
					...
					<span className="text-[10px] font-semibold" style={{ color: scopeColor }}>
						{SCOPE_LABEL_KEYS[rule.scope as ScopeKey] ? t(SCOPE_LABEL_KEYS[rule.scope as ScopeKey]) : rule.scope}
					</span>
				...
				>
					...
					<span className="text-muted-foreground ml-auto text-[10px]">
						{rule.targets.length === 1
							? t("routing.tree.targetCount", { count: rule.targets.length })
							: t("routing.tree.targetsCount", { count: rule.targets.length })}
					</span>
				</div>
					...
					>
							...
							<div className="mb-1 border-b px-3 pb-1.5">
								<p className="text-muted-foreground text-[10px]">
									<span className="font-semibold" style={{ color: scopeColor }}>
										{cfg ? t(SCOPE_LABEL_KEYS[rule.scope as ScopeKey]) : rule.scope}:{" "}
									</span>
						...
						{rule.chain_rule && (
							<div className="mb-1 flex items-start gap-2 border-b px-3 pb-1.5">
								...
								<p className="text-muted-foreground text-[10px] leading-snug">{t("routing.tree.chainRulePopover")}</p>
							</div>
						)}
						<p className="mb-1 px-3 text-[10px] font-semibold tracking-wide uppercase" style={{ color: scopeColor }}>
							{rule.chain_rule ? t("routing.tree.resolvedTarget") : t("routing.tree.targets")}
						</p>
