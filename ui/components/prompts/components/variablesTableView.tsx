...
export function VariablesTableView({
	...
	return (
		<div className="flex flex-col gap-3">
			<Label className="text-muted-foreground text-xs font-medium uppercase">{t("prompts.settings.variables")}</Label>
			<p className="text-muted-foreground text-xs">{t("prompts.settings.variablesHint")}</p>
			<div className="border-border overflow-hidden rounded-sm border">
				<table className="w-full table-fixed text-sm">
					<thead>
						<tr className="bg-muted/50 border-border border-b">
							<th className="text-muted-foreground w-[40%] max-w-[40%] px-3 py-1.5 text-left text-xs font-medium">
								{t("prompts.settings.variableColumn")}
							</th>
							<th className="text-muted-foreground px-3 py-1.5 text-left text-xs font-medium">{t("prompts.settings.valueColumn")}</th>
						</tr>
					</thead>
					<tbody>
							...
							<tr key={name} className="border-border border-b last:border-b-0">
								...
								<td className="py-1">
									<AutoSizeTextarea
										...
										placeholder={t("prompts.variableValuePlaceholder")}
										minRows={1}
